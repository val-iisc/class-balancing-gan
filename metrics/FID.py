...
def generate_images(batch_size, gen, dis, truncated_factor, prior, latent_op, latent_op_step,
                    latent_op_alpha, latent_op_beta, device, sampler = "default"):
    ...
    z, fake_labels = sample_latents(prior, batch_size, z_dim, truncated_factor, num_classes, None, device, sampler=sampler)
...
def get_activations(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior, is_generate,
                    latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable=False, run_name=None, sampler = "default", pretrained_classifier = None):
    ...
    if (sampler == "minority" or sampler == "majority" )and is_generate == False:
        total_instance = 5000
    ...
    pbar = tqdm(total = total_instance)
    ...
    while total_instance >0:
        if is_generate is True:
            ...
            images, labels = generate_images(batch_size, generator, discriminator, truncated_factor, prior, latent_op,
                                             latent_op_step, latent_op_alpha, latent_op_beta, device, sampler)
            ...
            if pretrained_classifier:
                labels = torch.argmax(pretrained_classifier(images), dim=1)
            
            if sampler == "minority" or sampler == "majority":
                    mid = (num_classes + 1) // 2 
                    if sampler == "minority":
                        starts, ends = mid, num_classes
                    else:
                        starts, ends = 0, mid
                    
                    inc_classes = [i for i in range(starts, ends)]
                    #selected_index = (labels[..., None] == inc_classes)
                    selected_index = torch.stack([labels == y_ for y_ in inc_classes]).sum(0).bool()
            ...
            try:
                ...
                if sampler == "minority" or sampler == "majority":
                    ...
                    images = torch.from_numpy(images.numpy()[np.isin(labels,inc_classes)])
                    labels = torch.from_numpy(labels.numpy()[np.isin(labels,inc_classes)])
...
def calculate_activation_statistics(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                                    is_generate, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable, run_name=None, sampler = "default", pretrained_classifier=None):
    act = get_activations(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                          is_generate, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable, run_name, sampler=sampler, pretrained_classifier=pretrained_classifier)
...
def calculate_fid_score(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                        latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, pre_cal_mean=None, pre_cal_std=None, run_name=None, sampler="default", pretrained_classifier=None):
    generator.eval()
    discriminator.eval()
    ...
    if pre_cal_mean is not None and pre_cal_std is not None:
        ...
        m1, s1 = calculate_activation_statistics(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor,
                                                 prior, False, False, 0, latent_op_alpha, latent_op_beta, device, tqdm_disable=False, sampler = sampler, run_name=run_name)
    ...
    m2, s2 = calculate_activation_statistics(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                                             True, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable=False, run_name=run_name, sampler=sampler, pretrained_classifier=pretrained_classifier)
