...
def loss_dcgan_dis(dis_out_real, dis_out_fake):
    ...
    dis_loss = -torch.mean(nn.LogSigmoid()(dis_out_real) + (ones - dis_out_fake.sigmoid() + 1e-6).log())
    return dis_loss

def loss_rel_dis(dis_out_real, dis_out_fake):
    
    #ones = torch.ones_like(dis_out_real, device=device, requires_grad=False)
    dis_loss = -torch.mean(nn.LogSigmoid()(dis_out_real - dis_out_fake))
    return dis_loss


def loss_dcgan_gen(gen_out_fake, gen_out_real):
    return -torch.mean(nn.LogSigmoid()(gen_out_fake + 1e-6))

def loss_rel_gen(gen_out_fake, gen_out_real):
    return -torch.mean(nn.LogSigmoid()(gen_out_fake - gen_out_real))
...
def latent_optimise(z, fake_labels, gen_model, dis_model, latent_op_step, latent_op_rate,
                    latent_op_alpha, latent_op_beta, trans_cost, default_device, scaler):
    batch_size = z.shape[0]
    for step in range(latent_op_step):
        drop_mask = (torch.FloatTensor(batch_size, 1).uniform_() > 1 - latent_op_rate).to(default_device)
        z_gradients, z_gradients_norm = calc_derv(z, fake_labels, dis_model, default_device, gen_model, scaler)
...
def calc_derv(inputs, labels, netD, device, netG=None, scaler = None):
    if netG is None:
        netD.eval()
